...
local function gotoNextDream()
  ...
  settings.currentDream = 7
  gotoDreaming()
end

local function gotoSameDream()
  audio.play( t_audio.confirm )
  settings.currentDream = 6
  gotoDreaming()
end
...
local t_phases = {
  playerTutorialPhase, spawnPhase, techniquesPhase, buffsPhase, battlePhase, playerAttackPhase, enemyAttackPhase, dreamCompleteCheckPhase
}
...
local function enemyAttack()
  ...
  if #t_cbtenemy.goons <= 0 then
    ...
    for i=1, #t_cbtenemy.goons do
      print("goon index: " .. t_cbtenemy.goons[i].index .. " attacks")
...
function scene:create( event )
  ...
  t_tutorial.playerTutorial = {
    "Gli Incubi stanno diventando sempre più potenti e numerosi", --1
    "Ma anche le fantasie di Aliza ora si sono scatenate",
    "Scegli 4 potenziamenti tra quelli offerti", --3
    "Miglioreranno una particolare caratteristica della fantasia scelta",
    "Difendi Aliza sfruttando al meglio questo tuo nuovo potere", --5
    "Combinare fantasie e potenziamenti è la chiave per la vittoria!"
  }
  ...
  t_goons.goonsToSpawn = 12
  t_goons.goonsSpawned = 0
  t_goons.goonsKilled = 0
  t_goons.tikeyInterval = 3
...
t_phases.playerTutorialPhase = function()
  ...
  local function narrateTutorial()
    local options = {
      ...
      text = t_tutorial.playerTutorial[t_tutorial.tutorialCounter - 4],
    ...
    if t_tutorial.tutorialCounter == 7 then
      for i=1, #t_buffs.buffs do
        spawnArrow( i, t_buffs.buffs[i].x + 88, t_buffs.buffs[i].y - 10, 90 )
      end
...
t_phases.spawnPhase = function()
  ...
  print("spawnPhase()")
  if t_goons.goonsToSpawn == 12 then
    spawnGoons( "tikey" )
    spawnGoons( "skeley" )
  end
  if t_goons.goonsToSpawn > 0 and t_goons.roseyInterval == 0 then
    ...
    t_goons.roseyInterval = t_goons.roseyInterval - 1
  end
...
t_phases.battlePhase = function()
  ...
  if t_tutorial.tutorialCounter > 4 then
    for i=1, 6 do
