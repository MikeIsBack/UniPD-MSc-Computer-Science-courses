...
local function gotoNextDream()
  ...
  settings.currentDream = 4
  gotoDreaming()
end

local function gotoSameDream()
  audio.play( t_audio.confirm )
  settings.currentDream = 3
  gotoDreaming()
end
...
local t_phases = {
  spawnPhase, techniquesPhase, playerAttackPhase, enemyAttackPhase, dreamCompleteCheckPhase
}
...
local function damagePlayer( damage )
  ...
  print("player lost " .. damage .. " fortitude")
...
local function enemyBulletCollision( self, event )
  ...
  if event.other.type == "player" then
    if self.type == "bone" then -- skeley
      ...
      damagePlayer( 2 + math.random( 0, 2 ) )
      display.remove( self )
      self = nil
    elseif event.other.type == "ground" then
      transition.to( self, { time=50, alpha=0, onComplete=function(self) display.remove(self); self=nil; end } )
  ...
  if #t_cbtenemy.goons <= 0 then
    ...
    for i=1, #t_cbtenemy.goons do
      ...
      if t_cbtenemy.goons[i].race == "skeley" then
        ...
        bullet:addEventListener("collision")
        bullet:applyLinearImpulse( -( math.random( 60, 66 ) / 10 ), math.random( -36, -28 ) / 10, bullet.x, bullet.y )
        transition.to( bullet, { rotation=1080, time=3000 } )
      end
    end
...
t_phases.spawnPhase = function()
  ...
  local function spawnGoons( race )
    ...
    if race == "tikey" then
      ...
      print("new goon added to index " .. #t_cbtenemy.goons)
      newgoon.x = math.random( display.contentCenterX + 80, display.contentCenterX + 115 )
    ...
    elseif race == "skeley" then
      ...
      t_goons.goonsSpawned = t_goons.goonsSpawned + 1
      print("new goon added to index " .. #t_cbtenemy.goons)
      newgoon.x = math.random( display.contentCenterX + 120, display.contentCenterX + 190 )
      newgoon.y = math.random( display.contentCenterY - 34, display.contentCenterY - 24 )
  ...
  if t_goons.goonsToSpawn ~= 0 and t_goons.goonsSpawned == t_goons.goonsKilled then
    spawnGoons( "tikey" )
    spawnGoons( "skeley" )
    t_goons.goonsInterval = 1
  elseif t_goons.goonsToSpawn ~= 0 and t_goons.goonsInterval == 0 then
    spawnGoons( "tikey" )
    spawnGoons( "skeley" )
    t_goons.goonsInterval = 1
  else
    t_goons.goonsInterval = t_goons.goonsInterval - 1
  end
  print("goonsSpawned: " .. t_goons.goonsSpawned)
  print("goonsToSpawn: " .. t_goons.goonsToSpawn)
  print("still " .. t_goons.goonsToSpawn .. " to spawn")
  print("goons spawning in mostly " .. t_goons.goonsInterval .. " turns")
...
t_phases.dreamCompleteCheckPhase = function()
  ...
  elseif t_goons.goonsKilled >= ( t_goons.goonsToSpawn + t_goons.goonsSpawned ) then
    ...
    timer.performWithDelay( 1000, t_phases.spawnPhase() )
  end
